...
from renderLibrary.utils import platforms
from renderLibrary.modules import readWrite
from renderLibrary.modules import studioMaya
from renderLibrary.modules import studioImage
from renderLibrary.modules import mayaNodes


class Render(studioMaya.Maya):

    def __init__(self, node=None, path=None):
        self.render_layer = node
        self.path = path
    ...
    def had_valid(self, publish_file):
        rw = readWrite.ReadWrite(t='render')
    ...
    def get_aovs_data(self):
        pass
              
    def get_node_data(self, node=None):
        if not node:
            node = self.render_layer
        attribute_data = {}
        attributes = node.listAttr(
