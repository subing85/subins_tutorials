...
class MainWindow(QtGui.QMainWindow):
    ...
    def setup_ui(self):
        ...
        # self.treewidget.itemClicked.connect(partial(self.load_current_folder, self.treewidget))
    ...
    def default_menu(self, parent):
        ...
        for index in range(len(order_data)):
            ...
            action_default.triggered.connect(partial(self.create_default, order_data[index]))
    ...
    def set_preference(self):
        ...
        self.library_path = self.rw.get_library_path()
        self.load_library_folders(self.treewidget)
    ...
    def create(self, input=None):
        ...
        if not input:
            ...
            if not ok:
                self.studio_print.display_warning('abort the folder creation!...')         
                return
        ...
        if not result:
            ...
            return
        self.load_library_folders(self.treewidget)
        self.studio_print.display_info('\"%s\" Folder create - success!...' % message)
    ...
    def rename(self):
        ...
        if not result:
            ...
            return
        self.load_library_folders(self.treewidget)
        self.studio_print.display_info('\"%s\" Rename folder - success!...' % message)
    ...
    def remove(self):
        ...
        self.studio_print.display_info('\"%s\" Remove folder - success!...' % message)
    ...
    def load_publish_to_layout(self, listwidget, items):
        ...
        for each_file in publish_files:
            ...
            if not os.path.isfile(icon_path):
                icon_path = os.path.join(resources.getIconPath(), 'unknown.png')
    ...
    def publish(self):
        ...
        if not current_items:
            ...
            self.studio_print.display_warning('Not found any folder selection.')
            return
        ...
        if not os.path.isdir(current_path):
            ...
            self.studio_print.display_warning('Not found such Publish directory!...%s' % current_path)
            return
        ...
        if not label:
            ...
            self.studio_print.display_warning('Not found the Name of the the Publish!...')
            return        # add condition for mutlipe object
        
        studio_asset = studioAsset.Asset(path=self.source_file_path, image=self.q_image)
        if studio_asset.had_file(current_path, label):
            ...
            self.studio_print.display_warning('Already a file with the same name in the publish')
            if replay != QtGui.QMessageBox.Yes:
                return
            
        user_comment = self.asset.textedit_history.toPlainText()
        result = studio_asset.save(current_path, label, user_comment=user_comment)
    ...
    def builds(self, tag, listwidge, *args):        
        ...
        self.asset.image_to_button(path=studio_asset.get_image(publish_paths[-1]))
        ...
        if tag == 'build':  
            if __name__ == '__main__':            
                if not self.maya_path:
                    ...
                    self.studio_print.display_warning('Please set the maya path!...')
                    return
                if not os.path.isdir(self.maya_path):     
                    QtGui.QMessageBox.warning(
                        self, 'Warning', 'Not such maya path!...\n%s' % self.maya_path, QtGui.QMessageBox.Ok)
                    self.studio_print.display_warning('Not such maya path!...\n%s' % self.maya_path)
                    return
                            
            if not self.create_type:
                QtGui.QMessageBox.warning(
                    self, 'Warning', 'Please set the create type [import or reference]!...', QtGui.QMessageBox.Ok)
                self.studio_print.display_warning('Please set the create type [import or reference]!...')
                returns             
                      
            if __name__ == '__main__':
                result = studio_asset.create(
                    'standalone', 'reference', maya_path=self.maya_path, output_path=self.output_path)
                
                self.studio_print.display_info('maya file created in - {}'.format(result))
            ...
            #===================================================================
            # if False in result:
            #     self.studio_print.display_warning('Build Failed!... %s' % result[False])
            # else:
            #     self.studio_print.display_info('Build Success!...')
            #===================================================================

    def rename_model(self, lineedit):
        studio_asset = studioAsset.Asset()
        if not self.currnet_publish:
            self.studio_print.display_warning('Not selected any shader or shader not valid.')
            return
